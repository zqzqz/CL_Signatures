...
void schemeD_init_keypair(schemeD_sk* sk, schemeD_pk *pk, uint32_t number_of_messages) {
    ...
    pk->W = malloc(sizeof(ECP2_BN462) * number_of_messages);
...
void schemeD_destroy_keypair(schemeD_sk* sk, schemeD_pk *pk) {
    ...
    memset(pk->W, 0, (pk->l * sizeof(ECP2_BN462)));
    free(pk->Z);
    free(pk->W);
}
...
void schemeD_generate_pk(schemeD_pk *pk, schemeD_sk *sk) {
    ECP2_BN462_generator(&pk->g_2);
    ECP_BN462_generator(&pk->g);

    ECP2_BN462_copy(&pk->Y, &pk->g_2);
    ECP2_BN462_copy(&pk->X, &pk->g_2);

    PAIR_BN462_G2mul(&pk->X, sk->x);
    PAIR_BN462_G2mul(&pk->Y, sk->y);

    for(int i = 0; i < pk->l; i++) {
        ECP2_BN462_copy(&pk->Z[i], &pk->g_2);
        PAIR_BN462_G2mul(&pk->Z[i], sk->z[i]);

        ECP2_BN462_copy(&pk->W[i], &pk->Y);
        PAIR_BN462_G2mul(&pk->W[i], sk->z[i]);
    }
}
...
void schemeD_sign(schemeD_sig *sig, BIG_464_60 *message, schemeD_sk *sk, csprng *prng) {
    ...
    BIG_464_60 x_plus_xym, x_times_y, xym;
...
int schemeD_verify(schemeD_sig *sig, BIG_464_60 *message, schemeD_pk *pk) {
    ...
    PAIR_BN462_ate(&rhs, &pk->g_2, &sig->c);
    PAIR_BN462_fexp(&rhs);
