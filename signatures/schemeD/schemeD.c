...
void schemeD_init_keypair(schemeD_sk* sk, schemeD_pk *pk, uint32_t number_of_messages) {
    ...
    pk->W = malloc(sizeof(ECP2_BLS12381) * number_of_messages);
...
void schemeD_destroy_keypair(schemeD_sk* sk, schemeD_pk *pk) {
    ...
    memset(pk->W, 0, (pk->l * sizeof(ECP2_BLS12381)));
    free(pk->Z);
    free(pk->W);
}
...
void schemeD_generate_pk(schemeD_pk *pk, schemeD_sk *sk) {
    ECP2_BLS12381_generator(&pk->g_2);
    ECP_BLS12381_generator(&pk->g);

    ECP2_BLS12381_copy(&pk->Y, &pk->g_2);
    ECP2_BLS12381_copy(&pk->X, &pk->g_2);

    PAIR_BLS12381_G2mul(&pk->X, sk->x);
    PAIR_BLS12381_G2mul(&pk->Y, sk->y);

    for(int i = 0; i < pk->l; i++) {
        ECP2_BLS12381_copy(&pk->Z[i], &pk->g_2);
        PAIR_BLS12381_G2mul(&pk->Z[i], sk->z[i]);

        ECP2_BLS12381_copy(&pk->W[i], &pk->Y);
        PAIR_BLS12381_G2mul(&pk->W[i], sk->z[i]);
    }
}
...
void schemeD_sign(schemeD_sig *sig, BIG_384_58 *message, schemeD_sk *sk, csprng *prng) {
    ...
    BIG_384_58 x_plus_xym, x_times_y, xym;
...
int schemeD_verify(schemeD_sig *sig, BIG_384_58 *message, schemeD_pk *pk) {
    ...
    PAIR_BLS12381_ate(&rhs, &pk->g_2, &sig->c);
