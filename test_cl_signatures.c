...
void test_scheme_A(csprng *prng) {
    ...
    BIG_464_60 message;
    BIG_464_60_random(message, prng);
    ...
    memset(message, 0, (sizeof(BIG_464_60)));
    assert(! schemeA_verify(&sig, message, &pk) );
...
void test_scheme_B(csprng *prng) {
    ...
    BIG_464_60 message, randomness;

    BIG_464_60_random(message, prng);
    BIG_464_60_random(randomness, prng);
...
void test_scheme_C(csprng *prng) {
    ...
    BIG_464_60 message[number_of_messages];
    ...
    //Negative test change message to 0
    memset(message, 0, number_of_messages * (sizeof(BIG_464_60)));
    assert(! schemeC_verify(&sig, message, &pk) );
